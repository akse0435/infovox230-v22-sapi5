...
// "Infovox230" or "Infovox" prefix and any language name already in it are
// redundant, and only letters, digits and single spaces survive. Any of the
// twelve language names is stripped, not just this voice's, so "Infovox230
...
void Catalog::apply_installed_selection(const std::wstring& install_dir)
{
    ...
    auto was_chosen = [&chosen](const Voice& v) {
        ...
        for (const std::string& name : chosen) {
            ...
            // writes; the name Windows shows ("Infovox230 Danish Female") is
...
int Catalog::find_by_name(const std::wstring& display_name) const
{
    ...
    for (size_t i = 0; i < voices_.size(); ++i) {
        ...
        const std::string old_name = "Infovox " + v.mode_key;
